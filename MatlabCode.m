...
[xpos, ypos, T, AUC0_logistic_train_class0] = perfcurve(trainY, 1-scores_logistic_train, 0);     % ROC curve against NO(=0) class
figure, plot(xpos, ypos)        % plot ROC
hold on
[xpos, ypos, T, AUC1_logistic_train] = perfcurve(trainY, scores_logistic_train, 1);     % ROC curve against YES(=1) class
...
%%%%%%% ROC ON Testing Data
[xpos, ypos, T, AUC0_logistic_test] = perfcurve(testY, 1-scores_logistic_test, 0);     % ROC curve against NO(=0) class
figure, plot(xpos, ypos)        % plot ROC
hold on
[xpos, ypos, T, AUC1_logistic_test] = perfcurve(testY, scores_logistic_test, 1);     % ROC curve against YES(=1) class
plot(xpos, ypos)        % plot ROC
xlim([-0.05 1.05]), ylim([-0.05 1.05])
xlabel('\bf FP rate'), ylabel('\bf TP rate')
...
[xpos2, ypos2, T0, AUC0] = perfcurve(testY, scores_logistic_test, 1);
[xpos3, ypos3, T1, AUC1] = perfcurve(testY, scores_logistic_after_lasso, 1);
figure;
plot(xpos2, ypos2), hold on, plot(xpos3, ypos3);
l = cell(1,2);
l{1}='Logistic Regression W/O Lasso Simplification'; l{2}='Logistic Lasso Regression';
legend(l,'Location','southeast');
xlim([-0.05 1.05]), ylim([-0.05 1.05])
...
CFM_nn = confusionmat(testYOneHotColumnBased(2, :), labels_nn,  'Order', [1, 0]);
accuracy_nn = sum(diag(CFM_nn))/sum(CFM_nn(:))
technique4_accuracy_class1 = accuracy_nn;
technique4_accuracy_class0 = accuracy_nn;

CFM_nn_8_3 = confusionmat(testYOneHotColumnBased(2, :), labels_nn_8_3,  'Order', [1, 0]);
